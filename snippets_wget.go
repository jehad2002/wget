...
func main() {
	...
	if isBackground {
		fmt.Println("Output will be written to \"wget-log\".")
		fh, err := os.OpenFile("wget-log", os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0o666)
		if err != nil {
			panic(err)
		}
		defer func() {
	...
	var datachunk int64 = 1024 * int64(rateLimitKb)
	...
	for range time.Tick(timelapse * time.Second) {
		fmt.Println(time.Since(lastStart))
		lastStart = time.Now()
		_, err := io.CopyN(io.Discard, resp.Body, datachunk)
		if err != nil {
			break
		}
	}
	...
	if resp.StatusCode == 200 && strings.Contains(resp.Header.Get("Content-Type"), "text/html") {
		...
		for {
			...
			for indx, attr := range token.Attr {
				if attr.Key == "src" && strings.Contains(attr.Val, "js") {
					token.Attr[indx].Val = strings.ReplaceAll(attr.Val, "http", "ptth")
					fmt.Println("Token String:", token.String(), "; URL: ", parsedURL)
				}
			}
